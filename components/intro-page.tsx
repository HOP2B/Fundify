...
const IntroPage: React.FC = () => {
  ...
  return (
    <div
      ref={containerRef}
      className="bg-[#E3E1DC] text-[#121212] font-sans selection:bg-[#374336] selection:text-white"
    >
      {/* Noise Overlay */}
      ...
      <div
        ref={loaderRef}
        className="fixed inset-0 bg-black z-[10000] flex justify-center items-center text-white"
      >
        <div className="loader-text font-bold text-[5vw] uppercase tracking-tighter">
          Fundify
        </div>
      ...
      <main className="relative z-10 bg-[#E3E1DC] mb-[100vh] shadow-[0_50px_100px_rgba(0,0,0,0.5)]">
        ...
        <section className="py-32 px-6 md:px-20 grid md:grid-cols-2 gap-16 max-w-[1800px] mx-auto">
          ...
          <div className="text-xl font-light leading-relaxed text-gray-700">
            <p className="mb-8 split-animate">
              {splitText(
                "We believe generosity can change lives. Fundify connects people with causes that matter.",
              )}
            </p>
              ...
              >
                <div className="card-inner w-[90%] h-full bg-[#1a1a1a] border border-white/10 grid md:grid-cols-[1fr_1.2fr] overflow-hidden shadow-2xl">
                  ...
                  <div className="overflow-hidden">
                    <img
                      src={card.img}
                      className="card-img w-full h-full object-cover transition-transform duration-1000 hover:scale-110"
                      alt={card.title}
                    />
                  </div>
                </div>
        ...
        <section className="py-40 text-center flex flex-col items-center px-6">
          <h2 className="text-3xl md:text-5xl mb-8 uppercase font-bold tracking-wide">
            Энэрэл бол үйлдэл
          </h2>
      ...
      <footer className="footer-sticky fixed bottom-0 left-0 w-full h-screen bg-[#111] text-white flex flex-col justify-center items-center z-0">
        <div className="footer-content relative z-10 text-center px-4 max-w-7xl mx-auto">
          <div className="text-xs uppercase tracking-[0.3em] mb-8 text-gray-500">
            Make a difference.
          </div>
          <a
            href="/"
